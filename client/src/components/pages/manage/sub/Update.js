...
export default function Update(props) {
    ...
    const location = useLocation(); // Access the location object passed from the QuestionList
    const data = location.state?.data; // Get the data from the state
    ...
    for (let i = 0; i < data.history.length; i++) {
        switch (data.history[i].subGroup) {
            case 'A':
                first.push({ id: Date.now() + i, text: data.history[i].subQ });
                break;
            case 'B':
                second.push({ id: Date.now() + i, text: data.history[i].subQ });
                break;
            case 'C':
                third.push({ id: Date.now() + i, text: data.history[i].subQ });
                break;
            default:
                break;
        }
    ...
    const [subquestionsFirst, setSubquestionsFirst] = useState(first);
    const [subquestionsSecond, setSubquestionsSecond] = useState(second);
    const [subquestionsThird, setSubquestionsThird] = useState(third);
    ...
    const onUpdate = () => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                axios
                    .post(`${config.serverURL}/api/question/${id}`, sendData)
                    .then(response => {
    ...
    return (
        <div style={{ 'textAlign': 'center', 'padding': '50px 0', 'backgroundColor': 'lightcyan' }}>
            ...
            >
                <Button style={{ width: '50%'}} color="success" onClick={onUpdate}>Update</Button>
